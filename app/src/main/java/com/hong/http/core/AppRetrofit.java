...
/**
 * Retrofit 网络请求
 * Created by upc_jxzy on 2016/7/15 11:39
 */
public enum AppRetrofit {
    ...
    public Retrofit getRetrofit(@NonNull String baseUrl, @Nullable String token, boolean isJson) {
        Log.i("=============>","getRetrofit baseUrl___________________"+baseUrl);
        this.token = token;
        String key = baseUrl + "-" + isJson;
        if (!retrofitMap.containsKey(key)) {
            createRetrofit(baseUrl, isJson);
            Log.i("=========", "getRetrofit: 新创建一个网络请求对象_________");
        }else{
            Log.i("=========", "getRetrofit: 不重新创建一个网络请求对象_________");
        }
...
//                        原来总是在url请求之后添加唯一id    uniqueLoginId = admin
...
//                                AppData.INSTANCE.getLoggedUser().getLogin())
            ...
            Log.d(TAG, request.url().toString()+"_______request中的url_______________________");

            //第二次请求，强制使用网络请求
            String forceNetWork = request.header("forceNetWork");
            //有forceNetWork且无网络状态下取从缓存中取
            Log.i("=============>","AppRetrofit 601 - apr getRetrofit"+forceNetWork+"________"+!StringUtils.isBlank(forceNetWork)+"[]"+!NetHelper.INSTANCE.getNetEnabled());
            if (!StringUtils.isBlank(forceNetWork) && !NetHelper.INSTANCE.getNetEnabled()) {
                request = request.newBuilder()
                        .cacheControl(CacheControl.FORCE_CACHE)
                        .build();
                Log.i("=============>","AppRetrofit 601 - apr getRetrofit"+11111);
            } else if("true".equals(forceNetWork)){
                request = request.newBuilder()
                        .cacheControl(CacheControl.FORCE_NETWORK)
                        .build();
                Log.i("=============>","AppRetrofit 601 - apr getRetrofit"+222222);
            }
